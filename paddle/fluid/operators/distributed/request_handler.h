...
class VarHandle {
 ...
 public:
  ...
  std::string String() const {
    ...
    s << " in_vars:[";
    for (auto var_name : in_vars_) {
      s << var_name;
      s << " ";
    }
    s << "], out_vars:[";

    for (auto var_name : out_vars_) {
      s << var_name;
      s << " ";
    }
    s << "], ep:[" << ep_ << "], status:[" << status_ << "]";
  ...
  std::string names() const { return out_vars_; }
  std::string name() const { return out_vars_[0]; }
